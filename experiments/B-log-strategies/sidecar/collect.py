...
from measure_overhead import ResourceTracker, strip_ansi, time_linear_scan
...
def collect_from_pods(strategy: str, namespace: str,
    ...
    """
    Retrieve filtered CSVs from all filter-agent DaemonSet pods via kubectl exec.
    """
    t0_wall = time.monotonic()
    t0_cpu  = resource.getrusage(resource.RUSAGE_SELF).ru_utime
    ...
    seen: set = set()
    for pod in pods:
        rows = _kubectl_read_csv(namespace, pod)
        for row in rows:
            try:
        ...
        print(f"  [kubectl] {pod}: {len(rows)} total rows in CSV "
              f"({sum(1 for r in rows if start_ns <= int(r.get('timestamp_ns',0)) <= end_ns)} in window)")

    all_rows.sort(key=lambda r: r["timestamp_ns"])

    wall_s = time.monotonic() - t0_wall
    cpu_s  = resource.getrusage(resource.RUSAGE_SELF).ru_utime - t0_cpu
    return all_rows, round(wall_s, 3), round(cpu_s, 3)
...
def main():
    ...
    if args.loki_only:
        ...
        collection_wall_s = round(rt.wall_s, 3)
        collection_cpu_s  = round(rt.cpu_s, 3)
        collection_method = "loki"
    else:
        rows, collection_wall_s, collection_cpu_s = collect_from_pods(
            args.strategy, args.namespace, start_ns, end_ns
        )
        if not rows:
    ...
    if args.prom_url:
        ...
        if overhead:
            ...
            print("  [WARN] Prometheus returned no data for filter-agent pods — "
                  "cpu_s / peak_mem_mb will be absent", file=sys.stderr)
    ...
    # ── Write metrics ──────────────────────────────────────────────────────────
    window_duration = args.end - args.start
    ...
    metrics = {
        ...
        **overhead,
        ...
        "loki_collection_wall_s": collection_wall_s,
        "loki_collection_cpu_s":  collection_cpu_s,
    }
