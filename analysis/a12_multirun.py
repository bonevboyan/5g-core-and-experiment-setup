"""12 - Multi-run aggregation: cross-run stability of the fault atlas.
...
turns the single-run binary atlas into a *confidence* atlas:
  ...
  * cell agreement  - fraction of runs matching the consensus, averaged over
                      all cells (the cross-run analogue of a10's threshold
...
Run after each run's own analysis has been generated, e.g.:
    FAULT_DATASET=C-fault-detection            python3 -m analysis.run_all
    FAULT_DATASET=C-fault-detection-run2       python3 -m analysis.run_all
    python3 -m analysis.a12_multirun           # auto-discovers run dirs

Or pass analysis dirs explicitly (each must contain fault_atlas.csv):
    python3 -m analysis.a12_multirun data/analysis data/analysis-C-fault-detection-run2
...
def _discover() -> list[Path]:
    """Canonical run + any data/analysis-C-fault-detection-run* dirs."""
    dirs = [OUT] + sorted(DATA_ROOT_DIR.glob("analysis-C-fault-detection-run*"))
...
def run(run_dirs: list[Path] | None = None) -> pd.DataFrame | None:
    ...
    if len(mats) < 2:
        print(f"[12] multirun: found {len(mats)} run(s) ({', '.join(labels) or 'none'}); "
    ...
    print(f"[12] multirun_atlas.csv  {n} runs ({', '.join(labels)})")
...
def _plot(freq: pd.DataFrame, layer_of: dict, n: int, overall: float) -> None:
    ...
    ax.set_title(f"Multi-run fault atlas — cell shade = fraction of {n} runs "
                 f"that fired (black=all, white=none); cross-run agreement "
                 f"{overall:.1f}%", fontsize=16, pad=58)
    fig.tight_layout()
