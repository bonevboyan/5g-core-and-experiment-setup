...
def _atlas_and_firstlayer(Z, persist, win, floor_m=1.0, ratio_m=1.0):
...
def run() -> pd.DataFrame:
    c.ensure_dirs()
    saved = (c.Z, c.PERSIST, c.WIN, c.FLOOR_MULT, c.RATIO_MULT)
    try:
        base_atlas, base_fl, slugs = _atlas_and_firstlayer(
            BASE["Z"], BASE["PERSIST"], BASE["WIN"])
        rows = []
        for name, Z, p, w, fm, rm in GRID:
            atlas, fl, _ = _atlas_and_firstlayer(Z, p, w, fm, rm)
        ...
        print(f"[10] sensitivity.csv  worst-case atlas agreement vs baseline: "
              f"{worst:.1f}%  (>~85% => conclusions threshold-robust)")
        return df
    finally:
        c.Z, c.PERSIST, c.WIN, c.FLOOR_MULT, c.RATIO_MULT = saved
