...
def _heatmap(tl: pd.DataFrame) -> None:
    ...
    fig.text(0.01, 0.005,
             "Cells: top = first-fire bucket (s after t0); bottom = signal.  "
             "'lo–hi' = detector reports onset within the bucket [lo, hi); width = signal's report step.  "
             "Prometheus / Loki / Jaeger / pod gauges step = 5 s; RTT step = 1 s (1 Hz ping); K8s events sub-second (point).",
