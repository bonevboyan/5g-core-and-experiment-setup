...
One detection pass per fault is computed here and consumed by every 01-08 script,
so the atlas / temporal / propagation / discriminability / etc. views never
...
# a10 can sweep them (the floor/ratio constants are the real decision boundary
...
# --------------------------------------------------------------------------- #
# Fault metadata  (curated from kind/chaos/*.yaml + research/methodology.md)
...
#   origin         : origin layer per methodology.md
...
#   Metrics flagged caveat=True (over-counting per EXTENSIONS.md 10.10) are
...
def _zscore_onset(series: pd.DataFrame, base_vals, t0: float,
    ...
    if len(base) < 3:
        return None
    bmean, bstd = float(base.mean()), float(base.std(ddof=0))
    ...
    thresh = max(Z * bstd, FLAT_REL * abs(bmean), floor, FLAT_EPS)
    ab = abs(bmean)
...
def _detect_loki(ctx: Ctx, sig: Signal) -> Hit:
    ...
    if pre is not None and not pre.empty:
        ...
        bmean, bstd = float(pb.mean()), float(pb.std(ddof=0))
    else:
        bmean, bstd = 0.0, 0.0
    ...
    thresh = max(Z * bstd, FLAT_REL * abs(bmean)) + LOKI_MIN_EXTRA
    hit = Hit()
    streak, start = 0, None
